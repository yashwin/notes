...
4) *position: absolute;*
  ...
  Elements with *absolute: fixed;* are always positioned relative to the nearest positioned ancestor (
...
5) *position: sticky;*
  ...
  Elements with *sticky: fixed;* are always positioned based on the user's scroll position.
